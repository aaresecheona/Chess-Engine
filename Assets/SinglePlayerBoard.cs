...
public class SinglePlayerBoard : MonoBehaviour
{
   ...
   void Start()
    {
        ...
        myChessEngine = new Engine(AllPieces, AllSquares);
    ...
    void EngineTurn()
    {
        var (eval, bestPiece, bestSquare) = myChessEngine.minimax(depth, -100000, 100000, !PlayerIsWhite);
        if (bestPiece.name == "King" && bestPiece.GetCurrentSquare().GetFile() == bestSquare.GetFile() + 2)
        {
            CastleShort(bestPiece);
        }
        else if (bestPiece.name == "King" && bestPiece.GetCurrentSquare().GetFile() == bestSquare.GetFile() - 2)
        {
            CastleLong(bestPiece);
        }
        else
        {
            if (bestPiece.name == "Pawn"){
                if(bestSquare.GetRank() == 7)
                {
                    bestPiece.Promote("Queen", whitePieces[4]);
                }
                else if(bestSquare.GetRank() == 0)
                {
                    bestPiece.Promote("Queen", blackPieces[4]);
                }
